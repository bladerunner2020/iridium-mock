...
describe('timers test', () => {
  ...
  it('IR.SetTimeout', () => {
    ...
    expect(IR.irTimers.indexOf(timer1)).not.toBe(-1);
    expect(IR.irTimers.indexOf(timer2)).not.toBe(-1);
    clock.tick(1000);
    expect(mock1).toBeCalled();
    expect(mock2).not.toBeCalled();
    expect(IR.irTimers.indexOf(timer1)).toBe(-1);
    expect(IR.irTimers.indexOf(timer2)).not.toBe(-1);
    clock.tick(1000);
    expect(mock1).toBeCalled();
    expect(mock2).toBeCalled();
    expect(IR.irTimers.indexOf(timer1)).toBe(-1);
    expect(IR.irTimers.indexOf(timer2)).toBe(-1);
  });
  ...
  it('IR.SetInterval', () => {
    ...
    expect(IR.irTimers.indexOf(timer)).not.toBe(-1);
    clock.tick(10000);
  ...
  it('IR.ClearTimeout', () => {
    ...
    clock.tick(1000);
    expect(mock).not.toBeCalled();
    expect(IR.irTimers.indexOf(timer)).toBe(-1);
  });
